...
from data_utils import get_tranform, get_augm_data_loaders_in_file, \
    AugmBBoxDatasetAll, prepare_batch_cae
from logging_conf import logger
from model_utils import get_pretrained_models
from cae_utils import BasicBlock, InvResNet, InvDenseNet121, \
...
def lr_finder(batch_size, alpha, end_lr=10, diverge_th=5, model_name='resnet',
    ...
    encoder, n_features = get_pretrained_models(model_name=model_name,
                                                pretrain=pretrain)
    ...
    if model_name == 'resnet':
        ...
        model = ConvolutionalAutoencoder(encoder, decoder,
                                         n_features=n_features, unpool_scale=8)

    elif model_name == 'dense':
        decoder = InvDenseNet121()
        model = ConvolutionalAutoencoder(encoder, decoder,
                                         n_features=n_features, unpool_scale=7)
...
def run_cae(batch_size=64, lr=3e-4, alpha=1e-3, cae_loss=nn.MSELoss(),
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        def load_best_model(engine):
            ...
            # un-normalize images
            orig_images.mul_(0.240).add_(0.389)
            decoded_images.mul_(0.240).add_(0.389)
