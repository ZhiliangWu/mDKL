...
from model_utils import get_model, EpochOutputStore, CheckPointAfter
...
def lr_finder(batch_size, alpha, end_lr=10, diverge_th=5, model_name='resnet',
    ...
    model = get_model(model_name, pretrain=pretrain, output_dim=4)
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        with lf.attach(trainer,
            ...
            trainer_with_lr_finder.run(train_evaluator_loader)
...
def run(batch_size=64, lr=3e-4, alpha=0.1, model_name='resnet', epoch=50,
        pretrain=True, fold_idx=0, device=torch.device('cpu'),
...
if __name__ == '__main__':
    ...
    for f in range(5):
        for a in [1e-5, 1e-4, 1e-3]:
            run(batch_size=bs, lr=lrt, alpha=a, model_name=m_name,
                pretrain=pt, epoch=n_epoch, fold_idx=f, device=dc,
                exp_name=exp, run_name=r_name, seed=sd)
            gc.collect()
