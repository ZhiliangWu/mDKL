...
def run(batch_size=64, lr=3e-4, alpha=0.0, num_gp=4, num_inducing=10,
    ...
    multi_inducing_points = torch.stack([inducing_points for _ in range(num_gp)], dim=0)

    # n_inducing_points = num_inducing * batch_size
    # multi_inducing_points = torch.randn(n_gp, n_inducing_points, dim_gp_in)
...
if __name__ == '__main__':
    ...
    r_name = f'{m_name}_svgp_albu'

    for f in range(5):
        run(batch_size=bs, lr=lrt, alpha=a, num_gp=n_gp, num_inducing=n_idc,
            dim_gp_in=dim_gp_input, model_name=m_name, pretrain=pt,
            epoch=n_epoch, fold_idx=f,  device=dc, exp_name=exp,
